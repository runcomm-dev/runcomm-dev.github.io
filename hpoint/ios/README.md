#  TouchAd SDK for H.Point 설치 가이드
...
* Xcode 26.0.1 Build, Minimum Deployment 14.0 입니다.
...
* @param navViewController: H.Point에서 직접 지정한 navigationController
*/
@objc public static func openTodayEarningMenu(_ isProd : Bool, _ cid : String, _ gender : String?, _ birthYear : String?, _ navViewController : UINavigationController)
...
* @param navViewController: H.Point에서 직접 지정한 navigationController
*/
@objc public static func openEarningMenu(_ isProd : Bool, _ cid : String, _ gender : String?, _ birthYear : String?, _ navViewController : UINavigationController)
```


## 오늘의 출석 화면 시작

*  아래는 오늘의 출석 화면 시작함수 예시입니다.
```
let isProd : Bool = true(상용 도메인) 또는 false(개발도메인)
let cid : String = "고객식별번호"
let gender : String = "M"
let birthYear : String = "1996" 
let navViewController : UINavigationController = self.navigationController

TASDKManager.openTodayEarningMenu(isProd, cid, gender, birthYear, navViewController)
```

## 포인트 모으기 화면 시작

*  아래는 포인트 모으기 화면 시작함수 예시입니다.
```
let isProd : Bool = true(상용 도메인) 또는 false(개발도메인)
let cid : String = "고객식별번호"
let gender : String = "M"
...
TASDKManager.openEarningMenu(isProd, cid, gender, birthYear, navViewController)
```
...
* XCode 시뮬레이터를 이용한 앱 개발시에는 x86_64 아키텍쳐 빌드가 포함된 SDK 로 빌드하여야 합니다.
