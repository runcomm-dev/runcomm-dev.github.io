...
* 사용자가 광고 이용 후 포인트 적립이 이루어집니다.
  ...
  ## 광고전송 SDK 주요기능
    ...
    * H.Point 통합 회원 가입 : H.Point를 이용하는 회원 대상
...
# TouchAd SDK For H.Point 구성

* H.Point 버전 터치애드 SDK에 대한 설명입니다.
* 터치애드 SDK For H.Point 앱은 안드로이드 스튜디오(Meerkat)로 개발되었습니다.
* SDK 결과물은 확장자 aar 형태로 별도 제공됩니다.
* 안드로이드 minSdkVersion : 26 , targetSdkVersion : 35, compileSdkVersion : 35 (으)로 빌드되었습니다.
...
* dependencies implementation 라이브러리 버전이 H.Point에서 정의한 동일 라이브러리 버전과 상이할 경우 SDK 담당자에게 내용 전달바랍니다.
...
dependencies {
    ...
    implementation 'com.google.firebase:firebase-analytics:32.3.1’
...
* 포인트 모으기 화면 시작
*/
fun  openEarningMenu(context: Context, isProd: Boolean, cid: String, gender:String?, birthYear: String?)
...
~~~
TouchAdPlatform.openEarningMenu(context, isProd, cid, gender, birthYear)
~~~
