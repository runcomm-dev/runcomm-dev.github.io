...
* 필독 : Xcode 15.2 빌드 부터 라이브러리 deployment target을 12.0으로 설정하는 내용이 추가되었습니다.
```
...
```
/**
* 출첵 플러스 ViewController
* 생성자 매개변수
* @param cid: KB 회원관리번호 (필수)
*/
@objc public class KBTodayEarningMenuViewController: UINavigationController
...
*  KB 앱 내에서 출첵 플러스 메뉴를 선택하면 약관동의 거치고 출첵 플러스 화면을 시작할때 호출합니다.

* 아래는 출첵 플러스 ViewController 호출 예시입니다.

* Swift
```
let navController = KBTodayEarningMenuViewController(cid: "회원관리번호")
self.present(navController, animated:true, completion: nil)
```

* Objective-C
```
KBTodayEarningMenuViewController* vc = [[KBTodayEarningMenuViewController alloc] initWithCid:@"회원관리번호"];
[self.navigationController presentViewController:vc animated:YES completion:nil];
```
...
* arm64  빌드 SDK :  폴더/ios_touchAd_sdk/TouchadSDK.framework
