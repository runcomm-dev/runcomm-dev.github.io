...
* 사용자가 광고 이용 후 포인트 적립이 이루어집니다.
  ...
  * 신한카드 올댓 통합 회원 가입 : 신한카드 올댓을 이용하는 회원 대상
...
#  TouchAd SDK For 신한카드 올댓 설치 가이드
...
## build.gradle 설정 
  ...
  1. **build.gradle(project)파일수정**
     * 광고Id를 가져와 '포인트줍줍+' 광고참여를 하기 위해 plugins에 com.google.gms.google-services를 추가합니다.
...
* '포인트줍줍+'(오퍼월 리스트) 화면 시작
*/
fun  openSHEarningMenu(context: Context, cid: String, gender: String, birthYear: String)

/**
* '포인트줍줍+'(오늘의 줍줍) 화면 시작
*/
fun  openSHBannerMenu(context: Context, cid: String, gender: String, birthYear: String)
...
*  birthYear = 출생연도(ex : 1996)

*  아래는 '포인트줍줍+'(오퍼월 리스트) 화면 시작함수 호출 예시입니다.

~~~
TouchAdPlatform.openSHEarningMenu(context, cid, gender, birthYear)
~~~
...
* 아래는 '포인트줍줍+'(오늘의 줍줍) 화면 시작함수 호출 예시입니다.

~~~
TouchAdPlatform.openSHBannerMenu(context, cid, gender, birthYear)
~~~
